...
class SecretsManager:
    ...
    def __init__(self, nomad_addr: Optional[str] = None, nomad_token: Optional[str] = None):
        """
        ...
        self.nomad_addr = nomad_addr or os.getenv("NOMAD_ADDR")
        self.nomad_token = nomad_token or os.getenv("NOMAD_TOKEN")
    ...
    def sync_secrets(self, job_name: str, secrets: Dict[str, str]) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"❌ Failed to sync secrets. Status: {response.status_code}, Body: {response.text}")
                return False
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env-file", required=True, help="Path to .env file containing secrets.")
    parser.add_argument("--job", required=True, help="Nomad job name (used as variable path).")
