...
@patch("mesh.infrastructure.provision_node.multipass.subprocess.run")
@patch("mesh.infrastructure.provision_node.multipass.os.remove")
@patch(
    "mesh.infrastructure.provision_node.multipass.click.echo"
)  # Mock click.echo to prevent output during tests
def test_generate_cloud_init_yaml(mock_click_echo, mock_os_remove, mock_subprocess_run):
    ...
    assert {
        ...
        "permissions": "0755",
        "content": boot_script,
    } in loaded_yaml["write_files"]


@patch("mesh.infrastructure.provision_node.multipass.subprocess.run")
@patch("mesh.infrastructure.provision_node.multipass.os.remove")
@patch("mesh.infrastructure.provision_node.multipass.click.echo")
def test_provision_multipass_node_launch(mock_click_echo, mock_os_remove, mock_subprocess_run):
    ...
    mock_subprocess_run.side_effect = [
        ...
        MagicMock(
            returncode=0, stdout=json.dumps({"info": {"test-mp-node": {"ipv4": ["10.0.0.5"]}}})
        ),  # 3. info after launch (get IP)
    ]
    ...
    result = provision_multipass_node(
        name=node_name, instance_size=instance_size, role=role, boot_script_content=boot_script
    )
    ...
    for call in mock_subprocess_run.call_args_list:
        ...
        if (
            len(args) > len(expected_launch_cmd_prefix)
            and args[: len(expected_launch_cmd_prefix)] == expected_launch_cmd_prefix
        ):
            launch_called = True
...
@patch("mesh.infrastructure.provision_node.multipass.os.remove")
@patch("mesh.infrastructure.provision_node.multipass.click.echo")
def test_provision_multipass_node_existing_running(
    mock_click_echo, mock_os_remove, mock_subprocess_run
):
    """
    ...
    mock_subprocess_run.side_effect = [
        MagicMock(
            returncode=0,
            stdout=json.dumps(
                {"info": {"existing-node": {"state": "Running", "ipv4": ["10.0.0.6"]}}}
            ),
        ),  # 1. info check
        # No launch, no start
        MagicMock(
            returncode=0, stdout=json.dumps({"info": {"existing-node": {"ipv4": ["10.0.0.6"]}}})
        ),  # 2. info get IP
    ]
...
@patch("mesh.infrastructure.provision_node.multipass.os.remove")
@patch("mesh.infrastructure.provision_node.multipass.click.echo")
def test_provision_multipass_node_existing_stopped(
    mock_click_echo, mock_os_remove, mock_subprocess_run
):
    """
    ...
    mock_subprocess_run.side_effect = [
        MagicMock(
            returncode=0,
            stdout=json.dumps({"info": {"stopped-node": {"state": "Stopped", "ipv4": []}}}),
        ),  # 1. info check
        MagicMock(returncode=0),  # 2. multipass start
        MagicMock(
            returncode=0, stdout=json.dumps({"info": {"stopped-node": {"ipv4": ["10.0.0.7"]}}})
        ),  # 3. info get IP
    ]
