...
# Default SSH users per provider
PROVIDER_SSH_USERS = {
    "digitalocean": "root",
    "do": "root",
    "aws": "ubuntu",
    "gcp": "ubuntu",  # or depends on image
    "azure": "azureuser",
    "linode": "root",
    "vultr": "root",
    "hetzner": "root",
...
def run_ssh(
    ...
    if user is None:
        user = PROVIDER_SSH_USERS.get(provider or "", "root")

    if demo:
        _run_ssh_demo(node_name=node_name, user=user)
        return
...
def _run_ssh_demo(
    ...
    if not node_name:
        ...
        table = Table(
            title=f"[bold {MESH_ORANGE}]{STATUS_ICONS['mesh']} Cluster Nodes (demo)[/]",
            border_style=MESH_DIM,
            show_header=True,
            header_style=f"bold {MESH_CYAN}",
            padding=(0, 1),
        ...
        console.print(table)
        console.print()
        show_info("Usage: mesh ssh <node_name> --demo")
        return
    ...
    if not target:
        show_error(
            f"Node '{node_name}' not found. Use 'mesh ssh --demo' to list available nodes."
        )
        raise typer.Exit(1)
    ...
    console.print(
        f"  {STATUS_ICONS['mesh']} [bold {MESH_CYAN}]Connecting to "
        f"[bold {MESH_ORANGE}]{node_name}[/] ({ip}) (demo)[/]"
    )
    console.print()
    console.print(f"  [dim]ssh {user}@{ip}[/dim]")
    console.print(
        f"  [dim]Welcome to Ubuntu 24.04 LTS (GNU/Linux 6.5.0-generic x86_64)[/dim]"
    )
    console.print()
    show_info("Demo mode — no real SSH connection established.")
