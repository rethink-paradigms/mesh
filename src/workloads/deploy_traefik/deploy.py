...
def deploy_traefik(
    ...
    cmd.extend([
        "-var", f"acme_email={acme_email}",
        "-var", f"acme_ca_server={acme_server_url}",
        "-var", f"use_tls_challenge={str(use_tls_challenge).lower()}",
        "-var", f"use_http_challenge={str(use_http_challenge).lower()}",
        "-var", f"acme_storage_path={acme_storage_path}",
        "-var", f"memory={memory}",
        "-var", f"cpu={cpu}",
        "-var", f"dashboard_enabled={str(dashboard_enabled).lower()}",
        "-var", f"log_level={log_level}",
        job_file
    ])
...
def main():
    ...
    parser.add_argument(
        "--no-tls-challenge",
        action="store_true",
        help="Disable TLS-ALPN-01 challenge"
    )
    parser.add_argument(
        "--acme-storage-path",
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="CPU allocation in MHz (default: 200)"
    )
    parser.add_argument(
        "--no-dashboard",
        action="store_true",
    ...
    parser.add_argument(
        "--nomad-addr",
        help="Nomad server address (default: http://127.0.0.1:4646)"
    )
