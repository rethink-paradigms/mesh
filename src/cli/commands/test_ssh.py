...
class TestSshNoCluster:
    @patch("src.cli.commands.ssh._check_cluster", return_value=False)
...
class TestSshListNodes:
    @patch("src.cli.commands.ssh._get_tailscale_ips", return_value={})
    @patch(
        "src.cli.commands.ssh._get_nodes",
        return_value=[
            {
                "id": "a1",
                "name": "mesh-leader",
    ...
    @patch("src.cli.commands.ssh._check_cluster", return_value=True)
    ...
    @patch("src.cli.commands.ssh._get_tailscale_ips", return_value={})
    @patch("src.cli.commands.ssh._get_nodes", return_value=[])
    @patch("src.cli.commands.ssh._check_cluster", return_value=True)
    ...
    @patch(
        "src.cli.commands.ssh._get_tailscale_ips",
...
class TestSshConnect:
    @patch("src.cli.commands.ssh.subprocess.Popen")
    @patch(
        "src.cli.commands.ssh._get_tailscale_ips",
        return_value={"mesh-leader": "100.64.0.1"},
    )
    ...
    @patch("src.cli.commands.ssh.subprocess.Popen", side_effect=FileNotFoundError)
    @patch("src.cli.commands.ssh._get_tailscale_ips", return_value={})
    @patch(
        "src.cli.commands.ssh._get_nodes",
        return_value=[
