...
from src.infrastructure.progressive_activation.tier_config import (
    TierConfig,
    ClusterTier,
)


@patch("src.infrastructure.provision_node.provision_node.UniversalCloudNode")
@patch(
    "src.infrastructure.provision_node.provision_node.multipass_provider.provision_multipass_node"
)
...
class TestLibcloudPathIntegration:
    ...
    @patch("src.infrastructure.provision_node.provision_node.UniversalCloudNode")
    @patch(
        "src.infrastructure.boot_consul_nomad.generate_boot_scripts.generate_shell_script"
    )
    @patch.dict(
        "os.environ",
        {"AWS_ACCESS_KEY_ID": "test-key", "AWS_SECRET_ACCESS_KEY": "test-secret"},
