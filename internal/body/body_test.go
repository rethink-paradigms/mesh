...
func (m *mockAdapter) ImportFilesystem(_ context.Context, _ adapter.Handle, _ io.Reader, _ adapter.ImportOpts) error {
	return nil
}
...
func TestMigrationDurability(t *testing.T) {
	...
	mc := NewMigrationCoordinator(s, ma, bm)
	...
	if rec.CurrentStep != 7 {
		t.Errorf("current_step = %d, want 7", rec.CurrentStep)
	}
	if rec.BodyID != b.ID {
		t.Errorf("body_id = %s, want %s", rec.BodyID, b.ID)
	}
...
func TestMigrationCreatesSnapshot(t *testing.T) {
	...
	migRec, err := s.GetMigration(ctx, migID)
	if err != nil {
		t.Fatalf("GetMigration: %v", err)
	}
	...
	if migRec.CurrentStep == 7 && len(snaps) != 0 {
